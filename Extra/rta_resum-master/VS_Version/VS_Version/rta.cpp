...
// constants
const double prefactor = 6. / (M_PI * M_PI);              // energy density prefactor
...
int main()
{
  ...
  char file[255] = "";
  sprintf_s(file, "tables/gauss_legendre_%dpts.dat", gauss_pts);
  FILE* gauss_file;
  fopen_s(&gauss_file,file, "r");
  if(gauss_file == NULL) printf("Error: couldn't open gauss legendre file\n");

  for(int i = 0; i < gauss_pts; i++)
  {
    fscanf_s(gauss_file, "%lf\t%lf", &root[i], &weight[i]);
  }
  fclose(gauss_file);
  //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
  ...
  if(!tau_R_constant)                                       // aniso hydro approximation
  {
    ...
    FILE *fp_temp0, *fp_e_aniso;
    // fp_temp0 = fopen("results/T_aniso.dat", "w");
    fopen_s(&fp_temp0, "results/T_aniso.dat", "w");
    fopen_s(&fp_e_aniso, "results/e_aniso.dat", "w");
    for(int i = 0; i < N_tau; i++)
    {
      fprintf(fp_temp0, "%.5f %.8f\n", tau[i], Temp[i]);
      fprintf(fp_e_aniso, "%.5f %.8f\n", tau[i], prefactor * pow(Temp[i], 4.0) * hbarc);
    }
    fclose(fp_temp0);
    fclose(fp_e_aniso);
  }
  ...
  FILE *fp_temp_ideal;
  fopen_s(&fp_temp_ideal,"results/T_ideal.dat", "w");
  for(int i = 0; i < N_tau; i++) fprintf(fp_temp_ideal, "%.5f %.8f\n", tau[i], T0 * pow(tau_min / tau[i], 1./3.));
  fclose(fp_temp_ideal);

  FILE *fp_e_ideal;
  fopen_s(&fp_e_ideal, "results/e_ideal.dat", "w");
  for (int i = 0; i < N_tau; i++) fprintf(fp_e_ideal, "%.5f %.8f\n", tau[i], prefactor * pow(T0, 4.0) * pow(tau_min / tau[i], 4.0 / 3.0) * hbarc);
  fclose(fp_e_ideal);
  ...
  //::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
  FILE *fp_temp, *fp_tau_R, *fp_z, *fp_e;
  FILE *fp_pibar, *fp_pibar_derivative, *fp_pibar_2nd_derivative, *fp_pibar_3rd_derivative, *fp_pibar_4th_derivative;
  fopen_s(&fp_temp, "results/T_exact.dat", "w");
  fopen_s(&fp_tau_R, "results/tau_r_exact.dat", "w");
  fopen_s(&fp_z, "results/z_exact.dat", "w");
  fopen_s(&fp_e, "results/e_exact.dat", "w");

  fopen_s(&fp_pibar, "results/pibar_exact.dat", "w");
  fopen_s(&fp_pibar_derivative, "results/pibar_derivative_exact.dat", "w");
  fopen_s(&fp_pibar_2nd_derivative, "results/pibar_2nd_derivative_exact.dat", "w");
  fopen_s(&fp_pibar_3rd_derivative, "results/pibar_3rd_derivative_exact.dat", "w");
  fopen_s(&fp_pibar_4th_derivative, "results/pibar_4th_derivative_exact.dat", "w");
