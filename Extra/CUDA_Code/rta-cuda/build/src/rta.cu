...
int main(int argc, char** argv) {
    ...
    cudaMemcpy(dev_t4, t4, sizeof(double)*num, cudaMemcpyHostToDevice);
    cudaMemcpy(dev_T4, T4, sizeof(double)*num, cudaMemcpyHostToDevice);
    cudaMemcpy(dev_time, t, sizeof(double)*num, cudaMemcpyHostToDevice);
    
    // copy parameters to device memory
    cudaMemcpyToSymbol(NUM, &num, sizeof(int));
    cudaMemcpyToSymbol(A_0, &a0, sizeof(double));
    cudaMemcpyToSymbol(T_0, &T0, sizeof(double));
    double eb = fpieb/M_PI/4.;
    cudaMemcpyToSymbol(EB, &eb, sizeof(double));
    cudaMemcpyToSymbol(DTAU, &dt, sizeof(double));
